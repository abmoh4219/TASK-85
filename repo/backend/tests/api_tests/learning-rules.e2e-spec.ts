...
describe('Learning & Rules Engine (e2e)', () => {
  ...
  it('LR-COV-1: GET /learning/plans returns plans list for current user', async () => {
    const res = await request(app.getHttpServer())
      .get('/learning/plans')
      .set('Authorization', `Bearer ${hrToken}`)
      .expect(200);
  ...
  it('LR-COV-2: GET /learning/plans/:id returns a single plan', async () => {
    const res = await request(app.getHttpServer())
  ...
  it('LR-COV-3: GET /learning/plans/:id/goals returns goals for a plan', async () => {
    const res = await request(app.getHttpServer())
      .get(`/learning/plans/${planId}/goals`)
      .set('Authorization', `Bearer ${hrToken}`)
      .expect(200);
    expect(res.body.data).toBeInstanceOf(Array);
  });

  it('RE-COV-1: GET /rules returns rules list (admin)', async () => {
    const res = await request(app.getHttpServer())
      .get('/rules')
      .set('Authorization', `Bearer ${adminToken}`)
      .expect(200);
  ...
  it('RE-COV-2: GET /rules/:id returns a single rule', async () => {
    const res = await request(app.getHttpServer())
      .get(`/rules/${ruleId}`)
      .set('Authorization', `Bearer ${adminToken}`)
      .expect(200);
