...
describe('Inventory (e2e)', () => {
  ...
  // ── Catalog management handlers (coverage) ────────────────────────────────

  it('GET /inventory/categories returns categories list', async () => {
    const res = await request(app.getHttpServer())
  ...
  it('PATCH /inventory/categories/:id updates a category', async () => {
    const createRes = await request(app.getHttpServer())
      .post('/inventory/categories')
      .set('Authorization', `Bearer ${adminToken}`)
      .set(nh())
      .send({ name: `${TEST_PREFIX}CatA_${Date.now()}`, description: 'initial' })
      .expect(201);
    const catId = createRes.body.data.id;

    const res = await request(app.getHttpServer())
      .patch(`/inventory/categories/${catId}`)
      .set('Authorization', `Bearer ${adminToken}`)
      .set(nh())
  ...
  it('POST /inventory/items creates a new item', async () => {
    const res = await request(app.getHttpServer())
      .post('/inventory/items')
      .set('Authorization', `Bearer ${adminToken}`)
      .set(nh())
      .send({
        name: `${TEST_PREFIX}Created Item ${Date.now()}`,
        sku: `${TEST_PREFIX}SKU-NEW1-${Date.now()}`,
        unitOfMeasure: 'each',
      })
      .expect(201);
  ...
  it('PATCH /inventory/items/:id updates an item', async () => {
    const res = await request(app.getHttpServer())
      .patch(`/inventory/items/${itemSafetyId}`)
      .set('Authorization', `Bearer ${adminToken}`)
      .set(nh())
  ...
  it('GET /inventory/recommendations returns recommendations list', async () => {
    const res = await request(app.getHttpServer())
  ...
  it('POST /inventory/recommendations/:id/dismiss dismisses a recommendation', async () => {
    // Generate a fresh reco to dismiss
    const genRes = await request(app.getHttpServer())
      .post('/inventory/recommendations/generate')
      .set('Authorization', `Bearer ${adminToken}`)
      .set(nh())
      .send({ itemId: itemConsumptionId });
    const freshRecoId = genRes.body.data[0]?.id ?? recoId;

    const res = await request(app.getHttpServer())
      .post(`/inventory/recommendations/${freshRecoId}/dismiss`)
      .set('Authorization', `Bearer ${adminToken}`)
      .set(nh());
