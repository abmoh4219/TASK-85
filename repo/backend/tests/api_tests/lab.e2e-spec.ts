...
describe('Lab (e2e)', () => {
  ...
  it('PATCH /lab/tests/:id updates a test dictionary entry', async () => {
    const res = await request(app.getHttpServer())
      .patch(`/lab/tests/${testId}`)
      .set('Authorization', `Bearer ${adminToken}`)
      .set(nh())
      .send({ description: 'Updated description for glucose test' });
    expect([200, 201]).toContain(res.status);
  });

  it('GET /lab/reports/:id returns report detail', async () => {
    const res = await request(app.getHttpServer())
