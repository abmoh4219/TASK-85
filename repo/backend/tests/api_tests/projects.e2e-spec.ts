...
describe('Projects (e2e)', () => {
  ...
  it('COV: GET /projects returns projects list', async () => {
    const res = await request(app.getHttpServer())
      .get('/projects')
      .set('Authorization', `Bearer ${adminToken}`)
      .expect(200);
  ...
  it('COV: GET /projects/:id/milestones returns milestones', async () => {
    const res = await request(app.getHttpServer())
  ...
  it('COV: GET /projects/:id/acceptance-score returns acceptance scores', async () => {
    const res = await request(app.getHttpServer())
      .get(`/projects/${projectId}/acceptance-score`)
      .set('Authorization', `Bearer ${adminToken}`)
      .expect(200);
    expect(res.body.data).toBeInstanceOf(Array);
  });
